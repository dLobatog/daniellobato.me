...
(() => {
  ...
  lesson('mle-map', 'CTR estimates: how much evidence earns trust?',
    ...
    'With 2/3 clicks and Beta(3,9), which value is the posterior mode?',
    [['4/13, about 0.308', true, 'Add alpha-1 and beta-1 to the likelihood counts for the mode.'], ['5/15, about 0.333', false, 'This is the posterior mean and the Bernoulli posterior predictive probability.'], ['2/3, about 0.667', false, 'This is MLE; it ignores the prior.']]);
  lesson('bias-variance', 'Fit all eight noisy datasets, not an invented U-curve',
    ...
    'The quadratic has zero training error on every dataset. What follows?',
    [['Its prediction can still vary across datasets.', true, 'Interpolation removes training residuals, not sensitivity to training noise.'], ['Its future prediction error must be zero.', false, 'Both fitted-model variance and future label noise remain.'], ['Variance means the spread of x within one dataset.', false, 'Here variance is over fitted predictions at the same test x.']]);
  lesson('regularization', 'Which feature survives the penalty?',
    ...
    'At lambda=0.5, why does L1 remove the second feature?',
    [['Its absolute OLS coefficient 0.4 is below the soft threshold.', true, 'The orthogonal-design solution is max(0.4-0.5,0)=0.'], ['L1 proves it has no causal effect.', false, 'Coefficient selection does not identify a causal effect.'], ['L2 also sets every coefficient below lambda to zero.', false, 'L2 divides by 1+lambda; it does not soft-threshold.']]);
  lesson('tree-split', 'Choose the split from actual class counts',
    ...
    'A tiny child is pure, but the other child stays mixed. Is that necessarily best?',
    [['No; child impurity is weighted by the number of rows.', true, 'A pure singleton cannot erase the impurity of the large child.'], ['Yes; one pure child is enough.', false, 'Compute both weighted terms before choosing.'], ['Choose the largest numeric threshold.', false, 'Feature magnitude does not measure split quality.']]);
  lesson('boosting', 'Fit the residuals, then add the leaf mean',
    ...
    'After the first update at rate 0.5, what does the second stump fit?',
    [['The new residuals y-F1.', true, 'Targets change after the ensemble update; they are recalculated from all previous trees.'], ['The unchanged original y.', false, 'That would not be residual boosting.'], ['Only the rows the previous tree classified incorrectly.', false, 'This is regression; every row contributes its signed residual.']]);
  lesson('feature-leakage', 'A temporal split cannot repair a future-aware join',
    ...
    'An event happened before the request, but landed in storage afterward. Can replay use it?',
    [['No; its availability time crosses the scoring boundary.', true, 'Event time alone is insufficient for point-in-time correctness.'], ['Yes; the event timestamp is old enough.', false, 'Serving could not yet read the feature.'], ['Yes, if the test split is chronological.', false, 'Splitting request rows does not fix a future-aware feature join.']]);
  lesson('feature-shift', 'Missing is not zero, and a schema is not a distribution',
    ...
    'After null-to-zero coercion, observed missingness is zero. Has the feed recovered?',
    [['Not necessarily; the transform may have erased the evidence.', true, 'Measure raw nulls before replacing them.'], ['Yes, because all values are numeric.', false, 'Schema validity says nothing about raw missingness.'], ['Accuracy must have fallen.', false, 'Without outcome labels, this input change does not establish a quality regression.']]);
  lesson('serving-skew', 'Replay the same request through both feature paths',
    ...
    'Both pipelines produce float32 interaction_count. What test detects the bug?',
    [['Replay identical requests and compare transformed values.', true, 'Type checks pass even when one path emits counts and the other emits logarithms.'], ['Only compare model checkpoint hashes.', false, 'The model is unchanged; its feature inputs differ.'], ['Retrain immediately on the buggy features.', false, 'First isolate and resolve the feature contract mismatch.']]);
  lesson('online-offline', 'The candidate wins offline, but which population?',
    ...
    'Reweighting flips the candidate difference from +3 to -11 percentage points. What is justified?',
    [['The aggregate comparison depends on cohort mix under the projection assumptions.', true, 'The same within-cohort rates can support opposite weighted averages.'], ['The candidate causally harms all users.', false, 'The logs are observational and the cohort differences have opposite signs.'], ['The offline calculation was arithmetically wrong.', false, 'It was correct for a different evaluation population.']]);
  lesson('data-drift', 'No feature alert. Is the model still healthy?',
    ...
    'Feature TV=0 and the positive label rate is unchanged. Can performance still collapse?',
  ...
  const initial = kind => ({
    ...
    regularization: { penalty: 'l1', lambda: 0.5, row: 0 }, 'tree-split': { mode: 'exact', threshold: 4.5, row: 2 },
  ...
  function reduce(kind, state, action, value) {
    ...
    if (action === 'primary') {
      ...
      const next = {
        'mle-map': ['item', (s.item + 1) % 3], 'bias-variance': ['sample', (s.sample + 1) % 8],
    ...
    const allowed = {
      ...
      regularization: { penalty: ['l1', 'l2'], lambda: [0, 0.25, 0.5, 1], row: [0, 1, 2, 3] },
  ...
  const flow = nodes => `<div class="tl-flow">${nodes.map(([label, value]) => `<div class="tl-flow-node"><span>${escape(label)}</span><strong>${escape(value)}</strong></div>`).join('')}</div>`;
  ...
  function renderMap(s) {
    ...
    return header(`How much should we trust ${k} clicks from ${n} impressions?`, 'The observed rate stays the same. More observations give it more weight.') +
      ...
      mechanism(flow([['Prior mode', s.prior === 'flat' ? 'No preferred rate' : '20.0%'], [`Observed: ${k}/${n}`, pct(result.mle)], ['MAP estimate', pct(result.map)]]) +
        ...
        `<div class="tl-rate-comparison">${bar('Observed', result.mle)}${bar('MAP', result.map, 'orange')}</div>`) +
      note(s.prior === 'flat' ? 'With no preferred prior rate, MAP equals the observed click fraction.' : `${n} impressions give the data ${pct(result.dataWeight)} of the MAP weight; the rest pulls toward the category's 20% prior mode.`) +
  ...
  function renderBias(s) {
    ...
    const all = weights.flatMap(w => grid.map(x => predictPolynomial(w, x))).concat(grid.map(x => x * x));
    ...
    const svg = `<svg class="tl-plot" viewBox="0 0 360 192" role="img" aria-label="Eight fitted curves, selected training observations, and true response x squared. Probe x=${s.probe}."><path class="tl-axis" d="M30 15V180H340"/>${weights.map((w, i) => `<path class="tl-line ${i === s.sample ? 'tl-stroke-orange' : 'tl-stroke-faint'}" d="${path(x => predictPolynomial(w, x))}"/>`).join('')}<path class="tl-line tl-stroke-blue" stroke-dasharray="5 4" d="${path(x => x * x)}"/><path class="tl-axis" stroke-dasharray="3 4" d="M${px(s.probe)} 18V180"/>${sampleX.map((x, i) => `<circle class="tl-fill-orange" cx="${px(x)}" cy="${py(samples[s.sample][i])}" r="4"/>`).join('')}<circle class="tl-fill-orange" cx="${px(s.probe)}" cy="${py(result.predictions[s.sample])}" r="5"/></svg><div class="tl-axis-labels"><span>x: -1.2</span><span>Probe: ${s.probe}</span><span>1.7</span></div><p class="tl-axis-note">Vertical range: ${fmt(lo, 1)} to ${fmt(hi, 1)}. Dotted line marks the probe.</p>`;
    const trainMSE = mse(samples[s.sample], sampleX.map(x => predictPolynomial(weights[s.sample], x)));
    return header('Would a new training sample change this prediction?', 'Same three inputs. Different +/-0.5 label noise. Fit the same model again.') +
      primary('Fit the next noisy sample', `Dataset ${s.sample + 1} of 8`) +
      mechanism(`<div class="tl-legend"><span class="tl-text-blue">Truth (dashed)</span><span class="tl-text-orange">Selected fit + data</span><span>Other fits (gray)</span></div><div class="tl-fit-layout"><div>${svg}</div><div class="tl-stats">${stat(`Prediction at x=${s.probe}`, fmt(result.predictions[s.sample]))}${stat('True response', fmt(result.truth))}${stat('This fit: training MSE', fmt(trainMSE))}</div></div>`) +
      note(`At the same test input, these eight fits predict ${fmt(Math.min(...result.predictions))} to ${fmt(Math.max(...result.predictions))}. ${s.degree === 2 ? 'Every fit has zero training error.' : 'Changing the training sample changes the fitted prediction.'}`) +
  ...
  function renderRegularization(s) {
    ...
    const names = ['Semantic match', 'Training-only correlate'];
    return header('Which feature survives the penalty?', `Two fitted weights. ${s.penalty.toUpperCase()} penalty, strength ${s.lambda}.`) +
      primary(s.penalty === 'l1' ? 'Compare L2 shrinkage' : 'Apply L1 instead') +
      mechanism(`<div class="tl-weight-key"><span>Before penalty</span><span>After ${s.penalty.toUpperCase()}</span></div>${[2, .4].map((before, j) => `<div class="tl-weight-lane"><div><span>${escape(names[j])}</span><strong>${fmt(before, 1)} to ${fmt(result.weights[j])}</strong></div><div class="tl-weight-track" role="img" aria-label="${escape(names[j])}: weight ${before} before, ${fmt(result.weights[j])} after"><i class="tl-weight-before" style="width:${before / 2 * 100}%"></i><i class="tl-weight-after" style="width:${result.weights[j] / 2 * 100}%"></i></div></div>`).join('')}<div class="tl-row-chips" role="group" aria-label="Inspect score contributions">${design.map((x, i) => tile('row', i, `R${i + 1}`, `${x[0]}, ${x[1]}`, i === s.row)).join('')}</div>${flow([['x1 contribution', fmt(row[0] * result.weights[0])], ['x2 contribution', fmt(row[1] * result.weights[1])], [`R${s.row + 1} score (sum)`, fmt(result.predictions[s.row])]])}`) +
      note(result.weights[1] === 0 ? 'L1 removes the small second weight here. It also shrinks the useful first weight.' : 'The second feature still contributes. Smaller coefficients alone do not guarantee better predictions.') +
      settings(s, controls('Exact penalty values', [0, 0.25, 0.5, 1].map(l => button('lambda', l, `lambda=${l}`, s.lambda === l)).join('')) + controls('Penalty', ['l1', 'l2'].map(p => button('penalty', p, p.toUpperCase(), s.penalty === p)).join(''))) +
  ...
  function renderTree(s) {
    ...
    const child = (label, rows) => `<section class="tl-tree-leaf"><h4>${escape(label)}</h4><strong>${sum(rows.map(row => row.y))}/${rows.length} clicks</strong><div class="tl-leaf-rows">${rows.map(row => tile('row', treeRows.indexOf(row), row.id, row.y, selected === row, row.y ? 'tl-positive' : '')).join('')}</div></section>`;
    return header('Which cut makes the click labels less mixed?', 'Eight fixed impressions. Move the boundary, not the data. Click a row to follow its route.') +
      ...
      mechanism(`<div class="tl-tree-root"><span>Parent: 5 clicks / 8 rows</span><strong>x <= ${s.threshold}?</strong></div><svg class="tl-tree-fork" viewBox="0 0 300 34" preserveAspectRatio="none" aria-hidden="true"><path class="tl-axis" d="M150 0V12H75V34M150 12H225V34"/></svg><div class="tl-tree-children">${child('Yes: left', r.left)}${child('No: right', r.right)}</div><div class="tl-mix-labels"><span>Row label: 1=click, 0=no click</span><strong>Gini gain ${fmt(r.gain)}</strong></div>`) +
      note(`${selected.id}: ${selected.x} interactions, label ${selected.y}; routes ${selected.x <= s.threshold ? 'left' : 'right'}. Weighted impurity falls from ${fmt(r.parent)} to ${fmt(r.weighted)}.`) +
  ...
  function renderBoost(s) {
    ...
    return header('What does the next tree change?', 'Each small tree fits the errors left by the current ensemble. Tap an example to follow its update.') +
      ...
      mechanism(`<div class="tl-legend"><span class="tl-text-blue">Target</span><span class="tl-text-orange">Current prediction</span><span>Height: 0 to 4</span></div><div class="tl-example-bars">${boostRows.map((row, j) => `<button type="button" data-action="row" data-value="${j}" aria-pressed="${s.row === j}" aria-label="${row.id}: target ${row.y}, prediction ${fmt(r.predictions[j])}"><span class="tl-bar-pair"><i class="tl-blue" style="height:${row.y / 4 * 100}%"></i><i class="tl-orange" style="height:${r.predictions[j] / 4 * 100}%"></i></span><span>${row.id}</span></button>`).join('')}</div>${flow([[`${boostRows[i].id} before`, fmt(s.step ? r.before[i] : r.predictions[i])], ['Tree correction', `${correction >= 0 ? '+' : ''}${fmt(correction)}`], ['After addition', fmt(r.predictions[i])]])}`) +
      note(`${boostRows[i].id} target=${boostRows[i].y}. ${s.step ? `The ${boostRows[i].x <= r.stump.threshold ? 'left' : 'right'} leaf contributes ${fmt(correction)} after multiplying its mean residual by ${s.rate}.` : 'Start from the mean target, 2.'} Training MSE=${fmt(r.mse)}.`) +
  ...
  function renderLeakage(s) {
    ...
    const stamps = before => events.filter(([, t]) => (t <= row.prediction) === before).map(([label, t]) => `<div class="tl-time-event"><span>${escape(label)}</span><strong>${t} min</strong></div>`).join('') || '<span class="ml-note">No event here</span>';
    return header('Could the model know this value when it scored?', `${row.id}: ${labels[s.field]}. The rule predicts this binary field, using 0 if it is unavailable.`) +
      ...
      mechanism(`<div class="tl-time-rail"><section><h4>Before scoring</h4>${stamps(true)}</section><section class="tl-time-boundary"><h4>Prediction</h4><strong>${row.prediction} min</strong></section><section><h4>After scoring</h4>${stamps(false)}</section></div>` +
        flow([['Snapshot value', row.field.value], ['Replay eligibility', row.available ? 'Available' : 'Too late'], [s.replay ? 'Replay prediction' : 'Hindsight prediction', s.replay ? row.replay : row.offline]]) +
        bar('Held-out accuracy', s.replay ? result.replayAccuracy : result.offlineAccuracy, 'orange')) +
      note(s.replay ? row.available ? 'This value was already readable. Replay keeps it.' : `Replay rejects the late value and uses 0. Held-out accuracy is ${pct(result.replayAccuracy)}, not the hindsight ${pct(result.offlineAccuracy)}.` : `The snapshot includes a value first readable at minute ${row.field.available}. ${row.available ? 'It passes the prediction-time check.' : 'The apparent score uses information that arrived too late.'}`) +
  ...
  function renderFeatureShift(s) {
    ...
    return header('Did missing data recover, or did we hide it?', 'Eight raw inputs pass through preprocessing. Tap one to inspect its model score.') +
      primary(s.policy === 'explicit' ? 'Replace nulls with zero' : 'Restore the missing flag') +
      mechanism(`<div class="tl-mix-labels"><span>Raw input to model input</span><span>* = missing flag retained</span></div><div class="tl-input-grid">${raw.map((v, j) => `<button type="button" class="tl-input-node${v === null ? ' tl-missing-node' : ''}" data-action="row" data-value="${j}" aria-pressed="${i === j}" aria-label="Slot ${j + 1}: raw ${v ?? 'null'}, model value ${v ?? 0}, missing flag ${Number(v === null && s.policy === 'explicit')}"><span>Slot ${j + 1}</span><strong>${v ?? 'null'}</strong><span class="tl-transform-arrow" aria-hidden="true">to</span><strong>${v ?? 0}${v === null && s.policy === 'explicit' ? '*' : ''}</strong></button>`).join('')}</div><div class="tl-stats">${stat('Raw missing rate', pct(live.missingRate))}${stat('After preprocessing', pct(live.visibleMissingRate))}${stat(`Slot ${i + 1}: model score`, pct(live.predictions[i]))}</div>`) +
      note(s.policy === 'zero' ? 'The nulls became zeros. The raw feed did not recover; the missingness signal was erased.' : 'The model can still distinguish a real zero from an imputed zero. No labels are available to measure accuracy.') +
  ...
  function renderSkew(s) {
    ...
    return header('Why does the same request get a different score?', `${row.id}: one interaction count, two feature paths, the same fixed model.`) +
      primary(s.mode === 'fixed' ? 'Reintroduce the raw-count bug' : 'Apply the shared feature code') +
      mechanism(`<div class="tl-pipeline"><h4>Offline: log1p, null defaults to 2</h4>${flow([['Raw count', row.count ?? 'null'], ['Model input', fmt(p.offline)], ['Predicted click rate', pct(p.offlineScore)]])}<h4>Online: ${escape({ raw: 'raw count; no log1p', stale: 'log1p of stale count', null: 'log1p; null defaults to 0', fixed: 'same code and fresh count' }[s.mode])}</h4>${flow([['Source count', (s.mode === 'stale' ? row.stale : row.count) ?? 'null'], ['Model input', fmt(p.online)], ['Predicted click rate', pct(p.onlineScore)]])}</div><div class="tl-row-chips" role="group" aria-label="Inspect a paired request">${servingRows.map((r, i) => tile('row', i, r.id, r.count ?? 'null', s.row === i)).join('')}</div>`) +
      note(`${pairs.filter(v => Math.abs(v.delta) > 1e-9).length}/5 requests fail feature parity. ${s.mode === 'fixed' ? 'The predictions now match without changing the model.' : s.mode === 'null' ? 'Tap U5: only the null case exposes this bug.' : 'A matching schema did not catch the different feature values.'}`) +
  ...
  function renderCohorts(s) {
    ...
    return header('Can a different audience reverse the winner?', 'Keep observed cohort rates fixed. Change only who counts more in the evaluation.') +
      ...
      mechanism(`<div class="tl-cohort-nodes">${cohorts.map((row, i) => `<button type="button" class="tl-cohort-node" data-action="cohort" data-value="${i}" aria-pressed="${s.cohort === i}"><span>${row.name} users</span><strong>${pct(mixes[s.mix][i])} of audience</strong><span>Observed difference: ${(row.candidate - row.baseline) > 0 ? '+' : ''}${fmt((row.candidate - row.baseline) / row.n * 100, 0)} pp</span></button>`).join('')}</div><div class="tl-evidence-mix" role="img" aria-label="Audience weights: returning ${pct(mixes[s.mix][0])}, new ${pct(mixes[s.mix][1])}"><span class="tl-blue" style="width:${mixes[s.mix][0] * 100}%"></span><span class="tl-orange" style="width:${mixes[s.mix][1] * 100}%"></span></div><div class="tl-rate-comparison">${bar('Baseline', selected.baseline)}${bar('Candidate', selected.candidate, 'orange')}</div><div class="tl-outcome"><span>Weighted candidate minus baseline</span><strong>${selected.delta >= 0 ? '+' : ''}${fmt(selected.delta * 100, 1)} pp</strong></div>`) +
      note(`Observed success: ${c.name.toLowerCase()} users favor ${c.candidate > c.baseline ? 'the candidate' : 'the baseline'}. Their audience weight is ${pct(weight)}. These are not causal effects.`) +
  ...
  function renderDrift(s) {
    ...
    return header('Do these inputs tell us whether the model still works?', 'The model still scores one bucket at 20% and the other at 80%. The outcome labels arrive later.') +
      ...
      mechanism(`<div class="tl-bucket-grid">${batch.map((counts, j) => `<button type="button" class="tl-bucket-node" data-action="bucket" data-value="${j}" aria-pressed="${i === j}"><span>${j ? 'High-score' : 'Low-score'} bucket: ${r.counts[j]} rows</span><strong>Predict ${j ? 'click' : 'no click'}</strong><span class="tl-label-dots" aria-label="${s.labels ? `${counts[1]} click labels out of ${r.counts[j]}` : 'Outcome labels pending'}">${Array.from({ length: r.counts[j] }, (_, n) => `<span class="tl-label-dot${s.labels && n >= counts[0] ? ' tl-click-dot' : ''}">${s.labels ? Number(n >= counts[0]) : '?'}</span>`).join('')}</span><span>${s.labels ? `${counts[1]}/${r.counts[j]} actually clicked` : 'Actual clicks unknown'}</span></button>`).join('')}</div><div class="tl-stats">${stat('Input shift (total variation)', pct(r.tv))}${stat('Accuracy: reference / live', s.labels ? `${pct(ref.accuracy)} / ${pct(r.accuracy)}` : '80.0% / unknown')}</div><p class="ml-note">${s.labels ? 'Each mark is one actual label: 1=click, 0=no click.' : 'Each question mark is one request awaiting its outcome label.'}</p>`) +
      note(s.labels ? explanations[s.scenario] : `${r.tv === 0 ? 'The input mix matches the reference.' : 'The input mix changed.'} That alone cannot tell us whether predictions are correct.`) +
  ...
  const api = { betaEstimate, relativeBeta, samples, sampleX, fitPolynomial, predictPolynomial, biasVariance, regularizedFit, design, regularizationY,
