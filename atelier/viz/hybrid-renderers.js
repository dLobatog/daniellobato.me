...
function renderBayes(stageRoot, section, state, context) {
  ...
  const steps = [
    { key: 'whole', label: '1. Whole world' },
    { key: 'routes', label: '2. Positive routes' },
    { key: 'crop', label: '3. Condition on +' },
  ];
  const activeStep = steps.find((step) => step.key === stageRoot.dataset.storyStep)?.key || 'routes';
  const activeIndex = steps.findIndex((step) => step.key === activeStep);
  const population = 1000;
  const count = (value) => Math.round(value * population);

  const root = buildHybridRoot(stageRoot, 'wide');
  buildHeader(
    root,
    'Bayes is a three-step picture',
    'First measure how much of the world belongs to H. Then mark every way a positive test can happen. Finally crop to only positive tests and ask what share still belongs to H.'
  );

  const story = buildCard(root, 'Tap through the story', 'hv-card-bayes-story');
  const stepper = html('div', 'hv-story-stepper');
  const prev = html('button', 'hv-story-arrow', 'Previous');
  prev.type = 'button';
  prev.disabled = activeIndex === 0;
  prev.addEventListener('click', (event) => {
    event.stopPropagation();
    if (activeIndex > 0) {
      stageRoot.dataset.storyStep = steps[activeIndex - 1].key;
      context.rerender?.();
    }
  });
  stepper.appendChild(prev);
  steps.forEach((step) => {
    const button = html('button', `hv-story-step${step.key === activeStep ? ' active' : ''}`, step.label);
    button.type = 'button';
    button.addEventListener('click', (event) => {
      event.stopPropagation();
      stageRoot.dataset.storyStep = step.key;
      context.rerender?.();
    });
    stepper.appendChild(button);
  });
  const next = html('button', 'hv-story-arrow', 'Next');
  next.type = 'button';
  ...
  story.appendChild(stepper);

  const stageWrap = html('div', 'hv-bayes-story-stage');
  story.appendChild(stageWrap);

  const inspector = buildInspector(story, { title: 'Bayes story', body: 'Move step by step through the picture.' });
  const inspectorTargets = [];

  if (activeStep === 'whole') {
    const svg = createSvg(stageWrap, '0 0 620 280');
    const x = 92;
    const y = 56;
    const w = 420;
    const h = 164;
    const hW = Math.max(34, w * prior);
    const notHW = Math.max(34, w - hW);
    addGrid(svg, x, y, w, h, 6, 4);
    inspectorTargets.push(
      addInteractiveBox(svg, {
        ...
        detail: {
          title: 'Hypothesis region H',
          body: `${pct(prior, 1)} of the whole world belongs to H before you see any evidence. That is the prior.`,
        },
      }),
      addInteractiveBox(svg, {
        x: x + hW,
        ...
        width: notHW,
        height: h,
        fill: 'rgba(110,165,201,0.08)',
        stroke: 'rgba(110,165,201,0.22)',
        detail: {
          title: 'Everything outside H',
          body: `${pct(1 - prior, 1)} of the world is ¬H. If H is rare, this region dominates before the test says anything.`,
        },
      })
    );
    addMeasureLine(svg, x, y - 18, x + hW, y - 18, `P(H) = ${pct(prior, 1)}`, x + hW / 2, y - 24);
    addMeasureLine(svg, x + hW, y - 18, x + w, y - 18, `P(¬H) = ${pct(1 - prior, 1)}`, x + hW + notHW / 2, y - 24);
    addText(svg, x + 12, y + h + 22, `H = ${count(prior)} / ${population}`, 'hv-svg-kicker');
    addText(svg, x + hW + 12, y + h + 22, `¬H = ${count(1 - prior)} / ${population}`, 'hv-svg-kicker');
    stageWrap.appendChild(html('p', 'hv-footnote', 'Step 1: before evidence, Bayes is just the prior split of the world.'));
    inspector.set(inspectorTargets[0].detail);
    wireInspector(inspectorTargets, inspector, 0);
    setTakeaway(context, `Before seeing the test, only ${pct(prior, 1)} of the world belongs to H.`);
    setMetrics(context, [
      ['Prior P(H)', pct(prior, 1)],
      ['H count', `${count(prior)}`],
      ['¬H count', `${count(1 - prior)}`],
    ]);
  } else if (activeStep === 'routes') {
    const svg = createSvg(stageWrap, '0 0 620 312');
    const x = 92;
    const y = 54;
    const w = 420;
    const h = 178;
    const hW = Math.max(34, w * prior);
    const notHW = Math.max(34, w - hW);
    const tpH = Math.max(12, h * sensitivity);
    const fpH = Math.max(12, h * falsePositiveRate);
    addGrid(svg, x, y, w, h, 6, 5);
    svg.appendChild(svgEl('rect', { x, y, width: hW, height: h, rx: 18, fill: 'rgba(201,169,110,0.08)', stroke: 'rgba(201,169,110,0.18)' }));
    svg.appendChild(svgEl('rect', { x: x + hW, y, width: notHW, height: h, rx: 18, fill: 'rgba(110,165,201,0.04)', stroke: 'rgba(110,165,201,0.12)' }));
    inspectorTargets.push(
      addInteractiveBox(svg, {
        x,
        y,
        width: hW,
        height: tpH,
        fill: 'rgba(201,169,110,0.50)',
        stroke: palette.accent,
        detail: {
          title: 'Positive tests that really came from H',
          body: `True-positive mass is ${pct(tpMass, 1)} of the world, or about ${count(tpMass)} out of ${population}.`,
        },
      }),
      addInteractiveBox(svg, {
        x: x + hW,
        ...
        detail: {
          title: 'Positive tests that came from ¬H',
          body: `False-positive mass is ${pct(fpMass, 1)} of the world, or about ${count(fpMass)} out of ${population}.`,
        },
      })
    );
    addMeasureLine(svg, x, y - 18, x + hW, y - 18, `P(H)`, x + hW / 2, y - 24);
    addMeasureLine(svg, x + hW, y - 18, x + w, y - 18, `P(¬H)`, x + hW + notHW / 2, y - 24);
    addMeasureLine(svg, x - 20, y, x - 20, y + tpH, `P(+|H) = ${pct(sensitivity, 1)}`, x - 30, y + tpH / 2 + 4, 'end');
    addMeasureLine(svg, x + w + 20, y, x + w + 20, y + fpH, `P(+|¬H) = ${pct(falsePositiveRate, 1)}`, x + w + 30, y + fpH / 2 + 4, 'start');
    addText(svg, x + 12, y + h + 22, `gold area = P(H∩+) = ${pct(tpMass, 1)}`, 'hv-svg-kicker');
    addText(svg, x + hW + 12, y + h + 22, `rose area = P(¬H∩+) = ${pct(fpMass, 1)}`, 'hv-svg-kicker');
    stageWrap.appendChild(html('p', 'hv-footnote', 'Step 2: every positive test comes from one of these two highlighted routes.'));
    inspector.set(inspectorTargets[0].detail);
    wireInspector(inspectorTargets, inspector, 0);
    setTakeaway(context, `A positive test pool is a mix: about ${count(tpMass)} true positives and ${count(fpMass)} false positives per ${population} cases.`);
    setMetrics(context, [
      ['True-positive mass', pct(tpMass, 1)],
      ['False-positive mass', pct(fpMass, 1)],
      ['Positive pool P(+)', pct(evidenceMass, 1)],
    ]);
  } else {
    const strip = html('div', 'hv-bayes-strip hv-bayes-strip-large');
    ...
    trueSegment.innerHTML = `<span class="hv-bayes-segment-label">really H</span><span class="hv-bayes-segment-value">${pct(posterior, 1)}</span><span class="hv-bayes-segment-caption">${count(tpMass)} of ${count(evidenceMass)} positives</span>`;
    ...
    falseSegment.innerHTML = `<span class="hv-bayes-segment-label">not H</span><span class="hv-bayes-segment-value">${pct(1 - posterior, 1)}</span><span class="hv-bayes-segment-caption">${count(fpMass)} of ${count(evidenceMass)} positives</span>`;
    stageWrap.appendChild(strip);
    strip.appendChild(trueSegment);
    strip.appendChild(falseSegment);
    const formula = html('p', 'hv-footnote');
    formula.innerHTML = `Posterior = <strong>true-positive mass / all positive mass</strong> = ${pct(tpMass, 1)} / ${pct(evidenceMass, 1)} = ${pct(posterior, 1)}.`;
    stageWrap.appendChild(formula);
    inspectorTargets.push(
      {
        node: trueSegment,
        detail: {
          title: 'Posterior share',
          body: `Among all positive tests, ${pct(posterior, 1)} really came from H. That is the posterior P(H|+).`,
        },
      },
      {
        node: falseSegment,
        detail: {
          title: 'What is left after conditioning',
          body: `${pct(1 - posterior, 1)} of the positive-test pool still comes from false positives, so a positive signal is not automatically certainty.`,
        },
      }
    );
    const statGrid = html('div', 'hv-bayes-measures');
    statGrid.innerHTML = `
      <div class="hv-bayes-measure"><span class="hv-inline-label">positive-test pool</span><strong>${count(evidenceMass)}</strong></div>
      <div class="hv-bayes-measure"><span class="hv-inline-label">true positives</span><strong>${count(tpMass)}</strong></div>
      <div class="hv-bayes-measure"><span class="hv-inline-label">posterior P(H|+)</span><strong>${pct(posterior, 1)}</strong></div>
    `;
    stageWrap.appendChild(statGrid);
    stageWrap.appendChild(html('p', 'hv-footnote', 'Step 3: conditioning means you keep only positive tests, then measure the gold share of that cropped strip.'));
    inspector.set(inspectorTargets[0].detail);
    wireInspector(inspectorTargets, inspector, 0);
    setTakeaway(
      context,
      posterior > 0.8
        ? `After conditioning on positive tests, ${pct(posterior, 1)} of what remains is truly H.`
        : posterior > 0.4
          ? `After conditioning on positives, the posterior is ${pct(posterior, 1)}, so false positives still matter.`
          : `Even after a positive test, only ${pct(posterior, 1)} of the cropped strip is truly H.`
    );
    setMetrics(context, [
      ['Posterior P(H|+)', pct(posterior, 1)],
      ['True positives', `${count(tpMass)}`],
      ['False positives', `${count(fpMass)}`],
    ]);
  }
}
...
export const hybridRenderers = {
  bayes: { rendererKind: 'diagram', viewerLayout: 'comparison', render: renderBayes },
  entropy: { rendererKind: 'hybrid', viewerLayout: 'comparison', render: renderEntropy },
  neuron: { rendererKind: 'hybrid', viewerLayout: 'wide', render: renderNeuron },
  'activation-basics': { rendererKind: 'plot', viewerLayout: 'comparison', render: renderActivationBasics },
  'output-functions': { rendererKind: 'diagram', viewerLayout: 'comparison', render: renderOutputFunctions },
  'forward-pass': { rendererKind: 'diagram', viewerLayout: 'wide', render: renderForwardPass },
  'chain-rule': { rendererKind: 'diagram', viewerLayout: 'comparison', render: renderChainRule },
  backprop: { rendererKind: 'diagram', viewerLayout: 'comparison', render: renderBackprop },
  'gradient-descent': { rendererKind: 'plot', viewerLayout: 'wide', render: renderGradientDescent },
  optimizers: { rendererKind: 'plot', viewerLayout: 'wide', render: renderOptimizers },
  'lr-schedule': { rendererKind: 'plot', viewerLayout: 'wide', render: renderLrSchedule },
  positional: { rendererKind: 'diagram', viewerLayout: 'comparison', render: renderPositional },
  'transformer-block': { rendererKind: 'diagram', viewerLayout: 'wide', render: renderTransformerBlock },
  attention: { rendererKind: 'diagram', viewerLayout: 'comparison', render: renderAttention },
  'kv-cache': { rendererKind: 'diagram', viewerLayout: 'comparison', render: renderKvCache },
  mdp: { rendererKind: 'diagram', viewerLayout: 'wide', render: renderMdp },
  diffusion: { rendererKind: 'diagram', viewerLayout: 'wide', render: renderDiffusion },
  'matrix-factorization': { rendererKind: 'diagram', viewerLayout: 'comparison', render: renderMatrixFactorization },
  'two-tower': { rendererKind: 'diagram', viewerLayout: 'comparison', render: renderTwoTower },
  'retrieval-funnel': { rendererKind: 'diagram', viewerLayout: 'wide', render: renderRetrievalFunnel },
  'tree-split': { rendererKind: 'diagram', viewerLayout: 'comparison', render: renderTreeSplit },
  boosting: { rendererKind: 'plot', viewerLayout: 'wide', render: renderBoosting },
};

export function isHybridViz(vizId) {
